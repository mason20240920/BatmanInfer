...
TEST(InterfaceKvcachesCall, Gpt2Model)
{
    ...
    for (int i = 0; i < input_vec.size(); ++i) {
        ...
        std::cout << "2kv_cache_id: " << kv_cache_id << std::endl;
    }
    ...
    for (int i = 0; i < input_vec.size(); ++i) {
        ...
        std::cout << "3kv_cache_id: " << kv_block_ids[i] << std::endl;
    }
    model_interface->bi_set_input(input_vec, kv_cache_id_map);
    ...
    output_vec.clear();
    model_interface->bi_run(output_vec, kv_block_ids, false);
    test_interface_kvcaches_call::find_and_print_top_n(output_vec, 100);
    test_interface_kvcaches_call::find_and_print_top_n_softmax_log(output_vec, 100);



    // // 模拟进行第二次推理，需要获取到上一步推理时生成的 kv_cache_id作为参数传递
