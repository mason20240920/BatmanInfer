 ...
 * @param weight_scales 权重每通道scale
 * @param M 矩阵维度
 * @param K
 * @param N
...
void gemmlowp_mixed_quantized(
    ...
    input.allocator()->init(
        BITensorInfo(input_shape, 1, BIDataType::QASYMM8_SIGNED, BIQuantizationInfo(input_scale, input_offset)));

    // 2. 创建权重tensor (QSYMM8_PER_CHANNEL)
    BITensorShape weights_shape(N, K); // [K, N]
    BITensor weights;
    // 为每个通道创建scale vector
    std::vector<float> weight_scales_vec(weight_scales, weight_scales + num_channels);
    weights.allocator()->init(BITensorInfo(
        weights_shape, 1, BIDataType::QASYMM8_SIGNED, BIQuantizationInfo(weight_scales_vec)));

    // 3. 创建中间输出tensor (S32)
    BITensorShape output_shape(N, M); // [M, N]
    BITensor output_s32;
    output_s32.allocator()->init(BITensorInfo(output_shape, 1, BIDataType::S32));
    ...
    gemmlowp_mm_score.configure(&input, &weights, nullptr, &output_s32);
    ...
    output_stage_info.output_data_type = BIDataType::S32;

    // // 计算输出量化参数
    // const float output_scale = input_scale * (*std::max_element(weight_scales, weight_scales + num_channels));
    // const int32_t output_offset = 0; // 对称量化输出
    // output_stage_info.gemmlowp_multiplier = static_cast<int32_t>(output_scale * (1 << 31));
    // output_stage_info.gemmlowp_shift = 31;
    // output_stage_info.gemmlowp_offset = output_offset; // 8. 创建最终输出tensor (QASYMM8)
    // BITensor final_output;
    // final_output.allocator()->init(BITensorInfo(
    //     output_shape,
    //     1,
    //     BIDataType::QASYMM8,
    //     BIQuantizationInfo(output_scale, output_offset)
    // ));
    // final_output.allocator()->allocate();
    ...
    // output_stage.run();
    // 配置S32中间结果的量化信息，把每个通道的整体scale设置好
    std::vector<float> final_scales(num_channels);
    for (int i = 0; i < num_channels; i++) {
        final_scales[i] = input_scale * weight_scales[i];
    }
    ...
    dq_dst0.allocator()->allocate();
    BINEDequantizationLayer dq0;
    // dq0.configure(&final_output, &dq_dst0);
...
TEST(GEMMLOWPCOMPARE, BASICGEMMLOWP) {
    ...
    const float input_scale = 0.015294118021048752f;
    const int8_t input_offset = 16;
    const float weights_scale[2] = {0.0118, 0.0252};

    gemmlowp_mixed_quantized(input_vec.data(), weighs_vec.data(), input_scale, input_offset, weights_scale, 2, 3, 2, 2);
}
