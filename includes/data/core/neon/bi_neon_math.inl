...
#include <data/core/utils/bi_math.hpp>
...
namespace BatmanInfer {
     ...
     * 对数多项式系数
     *
     * 对数函数 log(x) 是一个非线性函数，直接计算其值代价较高。为了优化性能，可以使用 多项式逼近 方法来快速计算近似值。常见的方法包括：
     * 泰勒展开（Taylor expansion）
     * 切比雪夫多项式（Chebyshev polynomials）
     * log(x) = c0 + c1x + c2x^2 + c3x^3 + ...
     *
     * 8 个 128 位向量的数组，每个向量存储一个多项式系数
     * 每个系数对应逼近对数函数的某一项
     *
     * vdupq_n_f32: 单一的 32 位浮点数（float）复制到 128 位寄存器中的每个元素位置，从而生成一个向量寄存器
     */
    ...
    inline float32x4_t vfloorq_f32(float32x4_t val) {
        // 生成一个 128 位宽的向量 CONST_1，其中每个元素都被初始化为 1.0f
        static const float32x4_t CONST_1 = vdupq_n_f32(1.f);

        // 使用 vcvtq_s32_f32 指令将输入向量 val 中的每个浮点数转换为整数（int32_t）
        // 这个转换会截断小数部分（即直接去掉小数部分，而不是四舍五入）
        const int32x4_t z = vcvtq_s32_f32(val);
        // 使用 vcvtq_f32_s32 指令将上一步得到的整数向量 z 转换回浮点数向量 r
        const float32x4_t r = vcvtq_f32_s32(z);

        // 代码实现了一个基于 NEON 的向量化
        // 将输入向量中的每个浮点数向下取整
        // vcgtq_f32 指令比较向量 r 和 val 中的每个元素
        // vsubq_f32 指令将向量 r 中的每个元素减去 1.0f
        // 使用 vbslq_f32 指令，根据比较结果从两个向量中选择值
    ...
    */
    ...
    inline float32x4_t vtanhq_f32(float32x4_t val) {
        ...
        float32x4_t den = vbslq_f32(vcgtq_f32(vabsq_f32(x), CONST_THR), vaddq_f32(exp2x, CONST_1),
                                    vsubq_f32(CONST_1, num));
        float32x4_t tanh = vbslq_f32(vcgtq_f32(vabsq_f32(x), CONST_THR), vmulq_f32(num, vinvq_f32(den)),
                                     vmulq_f32(x, den));
        return tanh;
    }
    ...
    template<BIRoundingPolicy policy>
    ...
    inline void convert_float32x4x4_to_uint8x16(const float32x4x4_t &in, uint8x16_t &out) {
        const auto low =
                vcombine_u16(vqmovn_u32(vconvert_to_uint<policy>(in.val[0])),
                             vqmovn_u32(vconvert_to_uint<policy>(in.val[1])));
        const auto high =
                vcombine_u16(vqmovn_u32(vconvert_to_uint<policy>(in.val[2])),
                             vqmovn_u32(vconvert_to_uint<policy>(in.val[3])));
    ...
    inline void convert_float32x4x4_to_int8x16(const float32x4x4_t &in, int8x16_t &out) {
        const auto low =
                vcombine_s16(vqmovn_s32(vconvert_to_int<policy>(in.val[0])),
                             vqmovn_s32(vconvert_to_int<policy>(in.val[1])));
        const auto high =
                vcombine_s16(vqmovn_s32(vconvert_to_int<policy>(in.val[2])),
                             vqmovn_s32(vconvert_to_int<policy>(in.val[3])));
    ...
    inline uint8x16_t convert_float_to_int<float32x4x4_t, uint8x16_t>(const float32x4x4_t &in) {
        uint8x16_t out;
#ifdef __aarch64__
        convert_float32x4x4_to_uint8x16<BIRoundingPolicy::TO_NEAREST_EVEN>(in, out);
#else  //  __aarch64__
    ...
    inline int8x16_t convert_float_to_int<float32x4x4_t, int8x16_t>(const float32x4x4_t &in) {
        int8x16_t out;
#ifdef __aarch64__
        convert_float32x4x4_to_int8x16<BIRoundingPolicy::TO_NEAREST_EVEN>(in, out);
#else  //  __aarch64__
...
/** Logarithm polynomial coefficients */
    ...
    inline float16x8_t vtaylor_polyq_f16(float16x8_t x, const std::array<float16x8_t, 8> &coeffs) {
        ...
        const float16x8_t res = vaddq_f16(vaddq_f16(A, vmulq_f16(B, x2)),
                                          vmulq_f16(vaddq_f16(C, vmulq_f16(D, x2)), x4));
        return res;
    }
