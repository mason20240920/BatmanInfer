...
#ifndef BATMANINFER_BI_NE_SYMM_HPP
#define BATMANINFER_BI_NE_SYMM_HPP
 ...
 */
    ...
    int16x8_t finalize_quantization_int16(
            int32x4x2_t &in_s32, int result_fixedpoint_multiplier, int32_t result_shift, int16x8_t min_s16,
            int16x8_t max_s16) {
        if (result_shift < 0) {
 ...
 */
    ...
    inline int16_t finalize_quantization_int16(
            int32_t in_value, int result_fixedpoint_multiplier, int32_t result_shift, int16_t min_s16,
            int16_t max_s16) {
        if (result_shift < 0) {
 ...
 * @return Dequantized values in a neon vector
 */
    inline float32x4x4_t vdequantize(const int16x8x2_t &qv, const BIUniformQuantizationInfo &qi) {
        const float scale = qi.scale;
        const float32x4_t vscale = vdupq_n_f32(scale);
        const float32x4x4_t vdequantized_input = {{
                                                          vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_low_s16(qv.val[0]))),
                                                                    vscale),
                                                          vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_high_s16(qv.val[0]))),
                                                                    vscale),
                                                          vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_low_s16(qv.val[1]))),
                                                                    vscale),
                                                          vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_high_s16(qv.val[1]))),
                                                                    vscale),
                                                  }};
        return vdequantized_input;
    }
 ...
 * @return A neon vector holding the quantized values
 */
    inline qsymm16x8x2_t vquantize_qsymm16(const float32x4x4_t &qv, const BIUniformQuantizationInfo &qi) {
        const float scale = qi.scale;
        BI_COMPUTE_ERROR_ON(scale == 0.f);
        const float32x4_t vinvscale = vdupq_n_f32(1.f / scale);
 ...
 */
    inline int32x4x2_t multiply_by_quantized_multiplier_2row(int32x4x2_t input, int32_t qmul, int32_t shift) {
        ...
        int32x4x2_t result;
        result.val[0] = rounding_divide_by_pow2(vqdmulhq_n_s32(vmulq_n_s32(input.val[0], one_shifted), qmul),
                                                right_shift);
        result.val[1] = rounding_divide_by_pow2(vqdmulhq_n_s32(vmulq_n_s32(input.val[1], one_shifted), qmul),
                                                right_shift);
...
#endif //BATMANINFER_BI_NE_SYMM_HPP
