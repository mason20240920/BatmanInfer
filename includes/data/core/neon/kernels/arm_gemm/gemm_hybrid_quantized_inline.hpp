...
namespace BatmanGemm {
    ...
    class GemmHybridQuantizedInline : public BIGemmCommon<To, To, Tr> {
        ...
        static unsigned int compute_n_block(const GemmArgs &args) {
            ...
            unsigned int n_block = (((L2_size * 9) / 10) -
                                    (k_block * sizeof(Toi) * (strategy::out_width() + strategy::out_height()))) /
                                   (sizeof(Toi) * k_block);
    ...
    public:
...
#endif
                        strat.kernel(this->_Aptr + (multi * this->_A_multi_stride) + (batch * this->_A_batch_stride) +
                                     (m_start * this->_lda) + k0, this->_lda,
                                     b_panel,
                                     this->_Cptr + (multi * this->_C_multi_stride) + (batch * this->_C_batch_stride) +
                                     (m_start * this->_ldc) + n0, this->_ldc,
        ...
        void requantize_bias(void *in_buffer, const To *B, const int ldb, const int B_multi_stride) override {
            ...
            for (unsigned int i = 0; i < _nmulti; i++) {
                compute_col_sums(_qp, _Nsize, _Ksize, B + (i * B_multi_stride), ldb, col_bias + (i * _Nsize), _Ksize, i,
                                 0);
            }
        }
