...
// These must be included with a "namespace BatmanGemm" block.
    ...
    for (unsigned int ybase = y0; ybase < ymax; ybase += height) {
        ...
        while (k_left > 0) {
            ...
            if (std::is_integral<TOut>::value && integrate_sums && row_sum_multiplier) {
                interleave_block<height_vectors, block, vlt, true>(out, row_base, in_width, active_height, stringpos,
                                                                   first);
            } else {
                interleave_block<height_vectors, block, vlt, false>(out, row_base, in_width, active_height, stringpos,
                                                                    first);
            }
...
void ConvolutionInterleave(TOut *out, const TIn *in, size_t in_stride, const convolver <TIn> &conv,
                           ...
                           const unsigned int y0, const unsigned int ymax, const unsigned int k0,
                           const unsigned int kmax, bool integrate_sums, const int32_t row_sum_multiplier) {
    const unsigned int height = height_vectors;
...
void Interleave(TOut *out, const TIn *in, size_t in_stride, const unsigned int y0, const unsigned int ymax,
                const unsigned int k0, const unsigned int kmax, bool integrate_sums, const int32_t row_sum_multiplier) {
    const unsigned int height = height_vectors;
    // Use alloca here as a std::vector can be expensive in highly threaded scenarios.
    const TIn **row_ptrs = reinterpret_cast<const TIn **>(alloca(height * sizeof(const TIn *)));
    ...
    for (unsigned int y = y0; y < ymax; y += height) {
        ...
        if (std::is_integral<TOut>::value && integrate_sums && row_sum_multiplier) {
            interleave_block<height_vectors, block, vlt, true>(out, row_ptrs, width, std::min(height, ymax - y), k0,
                                                               true);
        } else {
            interleave_block<height_vectors, block, vlt, false>(out, row_ptrs, width, std::min(height, ymax - y), k0,
                                                                true);
        }
