...
// Implementation of the BIGemmCommon abstract class.
//
...
// KernelWeightFormat is a similar story.
        ...
        struct get_kernel_weight_format<strategy, true, Tro> {
            static KernelWeightFormat get() {
                ...
                // weight format.
                if (std::is_same<Tro, float>::value &&
                    std::is_same<typename strategy::rhs_operand_type, bfloat16>::value) {
    ...
    class GemmInterleaved : public BIGemmCommon<Tlo, Tro, Tr> {
        ...
        size_t get_accumulation_buffer_size() const {
            ...
            size_t num_buffers =
                    iceildiv(_Msize, strategy::out_height()) * iceildiv(_Nsize, strategy::out_width()) * _nbatches *
                    _nmulti;
    ...
    public:
        ...
        void execute_stateless(const ndcoord_t &work_range, const ndcoord_t &, int threadid,
                               BIGemmArrays<Tlo, Tro, Tr> &) override {
            ...
            if (_thread_columns) {
                ...
                Tloi *const a_panel = reinterpret_cast<Tloi *>(working_space_bytes +
                                                               (_maxthreads * get_c_working_size()) +
                                                               (threadid * sizeof(Tloi) * get_total_k_depth() *
                                                                strategy::out_height()));
                            ...
                            // Set up transposed 'A' block
                            {
                                ...
                                if (_indirect_buf != nullptr) {
                                    transforms.PrepareA_indirect(a_panel,
                                                                 _indirect_buf + (multi * _nbatches * _Ksections) +
                                                                 (batch * _Ksections), _Ksize,
                                                                 _rounded_Ksize, start_row, end_row, k0, kmax,
                                                                 row_sum_multiplier());
                                } else if (_convolver) {
                                    transforms.PrepareA_convolution(a_panel,
                                                                    g_array._Aptr + (batch * g_array._A_batch_stride) +
                                                                    (multi * g_array._A_multi_stride),
                                                                    g_array._lda, *_convolver, _rounded_Ksize,
                                                                    start_row, end_row, k0, kmax, row_sum_multiplier());
                                } else {
                                    transforms.PrepareA(a_panel,
                                                        g_array._Aptr + (batch * g_array._A_batch_stride) +
                                                        (multi * g_array._A_multi_stride),
                                                        g_array._lda, start_row, end_row, k0, std::min(kmax, _Ksize),
                                                        row_sum_multiplier());
                                }
                            }

                            Tr *result_ptr = g_array._Cptr + (batch * g_array._C_batch_stride) +
                                             (multi * g_array._C_multi_stride);
                ...
                for (; !current.done(); current.advance()) {
                        ...
                        for (unsigned int batch = batch_0; batch <= batch_end; batch++) {
                            ...
                            if (_indirect_buf != nullptr) {
                                ...
                                transforms.PrepareA(a_panel + ((batch * _Mround + first_m) * get_total_k_depth()),
                                                    g_array._Aptr + (batch * g_array._A_batch_stride) +
                                                    (current.multi() * g_array._A_multi_stride),
                                                    g_array._lda, first_m, last_m, current.k0(),
                                                    std::min(_Ksize, current.kmax()), row_sum_multiplier());
                            }
                        }
                    ...
                    if (FixedFormat) {
                        b_panel = reinterpret_cast<const Troi *>(g_array._Bptr) +
                                  (current.multi() * g_array._B_multi_stride) +
                    ...
                    for (unsigned int batch = batch_0; batch <= batch_end; batch++) {
                        ...
                        for (unsigned int y = first_m; y < last_m; y += m_step) {
                            ...
                            Tr *result_ptr = g_array._Cptr + (batch * g_array._C_batch_stride) +
                                             (current.multi() * g_array._C_multi_stride);
        ...
        void requantize_bias(void *in_buffer, const Tro *B, const int ldb, const int B_multi_stride) override {
            if (std::is_same<OutputStage, Requantize32>::value) {
                ...
                for (unsigned int i = 0; i < _nmulti; i++) {
                    ...
                    compute_col_sums(*qp_ptr, _Nsize, _Ksize * _Ksections, B + (i * B_multi_stride), ldb,
                                     col_bias + (i * _Nsize), _Ksize * _Ksections, i, 0);
                }
            }
        }
        ...
        void pretranspose_B_array(void *in_buffer, const Tro *B, const int ldb, const int B_multi_stride,
                                  const bool transposed) override {
            pretranspose_B_array_part(in_buffer, B, ldb, B_multi_stride, transposed, 0,
                                      get_B_pretranspose_window_size());
        }

        void pretranspose_B_array_part(void *in_buffer, const Tro *B, const int ldb, const int B_multi_stride,
                                       const bool transposed, size_t start, size_t end) override {
        ...
        static uint64_t estimate_cycles(const GemmArgs &args) {
            ...
            uint64_t total_macs = static_cast<uint64_t>(args._nbatches) * args._nmulti *
                                  roundup(args._Msize, strategy::out_height()) *
                                  roundup(args._Nsize, strategy::out_width()) * get_ktotal(args);
            uint64_t prepare_bytes = static_cast<uint64_t>(args._nbatches) * args._nmulti *
                                     roundup(args._Msize, strategy::out_height()) * get_ktotal(args) * sizeof(Tloi);
            uint64_t merge_bytes = static_cast<uint64_t>(args._nbatches) * args._nmulti * k_blocks * args._Msize *
                                   roundup(args._Nsize, strategy::out_width()) * sizeof(Tr);
            ...
            float parallelism_available =
                    static_cast<float>(iceildiv(args._Msize, strategy::out_height()) * args._nbatches) * 0.9f;
        ...
        GemmConfig get_config() override {
            ...
            c.weight_format = get_weight_format(get_kernel_weight_format<strategy, FixedFormat, Tro>::get(),
                                                sizeof(Tro));
