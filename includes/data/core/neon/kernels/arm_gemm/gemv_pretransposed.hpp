...
namespace BatmanGemm {
    ...
    //
    // This is implementation is for GEMV with pre-transposition.
    //
    ...
    class GemvPretransposed : public BIGemmCommon<To, To, Tr> {
    ...
    public:
        ...
        GemvPretransposed(const GemmArgs &args, const OutputStage &os = {})
                : _args(args),
                  _buffer_per_multi(
                          roundup(args._Ksize, strategy::k_unroll()) * roundup(args._Nsize, strategy::out_width())),
                  _os(os) {
                ...
                for (unsigned int k0 = 0; k0 < _args._Ksize; k0 += k_block) {
                    ...
                    for (unsigned int n = n_start; n < n_end; n += n_block) {
                        ...
                        run_gemv_kernel<OutputStage>::run(strat, g_array._Aptr + (multi * g_array._A_multi_stride) + k0,
                                                          _B_pretransposed + (multi * _buffer_per_multi) +
                                                          (n * roundup(_args._Ksize, strategy::k_unroll())) +
                                                          (k0 * strategy::out_width()),
                                                          g_array._Cptr + (multi * g_array._C_multi_stride) + n,
                                                          (nmax - n), (kmax - k0),
                                                          g_array._bias ? g_array._bias +
                                                                          (multi * g_array._bias_multi_stride) + n
                                                                        : nullptr,
        ...
        void requantize_bias(void *in_buffer, const To *B, const int ldb, const int B_multi_stride) override {
            ...
            if (std::is_same<OutputStage, Requantize32>::value) {
                ...
                for (unsigned int i = 0; i < _args._nmulti; i++) {
                    compute_col_sums(*qp_ptr, _args._Nsize, _args._Ksize, B + (i * B_multi_stride), ldb,
                                     col_bias + (i * _args._Nsize), _args._Ksize, i, 0);
                }
            }
        }
        ...
        void pretranspose_B_array(void *buffer, const To *B, const int ldb, const int B_multi_stride,
            ...
            for (unsigned int multi = 0; multi < _args._nmulti; multi++) {
                strat.transforms.PrepareB(B_buffer + (multi * _buffer_per_multi), B + (multi * B_multi_stride), ldb, 0,
                                          _args._Nsize, 0, _args._Ksize, false);
            }
