...
#include "graph/backends/bi_fusedConvolutionBatchNormalizationFunction.h"
#include "graph/backends/bi_fusedDepthwiseConvolutionBatchNormalizationFunction.h"
...
namespace detail {
    ...
    typename TargetInfo::TensorType *get_backing_tensor(BatmanInfer::graph::BITensor *tensor)
    {
        typename TargetInfo::TensorType *backing_tensor = nullptr;
        if (tensor != nullptr)
        {
            BI_COMPUTE_ERROR_ON(tensor->desc().target != TargetInfo::TargetType);
            // Get backing tensor handle
            BIITensorHandle *tensor_handle = tensor->handle();
            // Get backing tensor
            backing_tensor = (tensor_handle != nullptr)
                                 ? BatmanInfer::utils::cast::polymorphic_cast<typename TargetInfo::TensorType *>(
    ...
    template <typename TargetInfo>
    void validate_node(const BIINode &node, size_t num_expected_inputs, size_t num_expected_outputs)
    {
        BI_COMPUTE_LOG_GRAPH_VERBOSE("Creating " << node.type() << " Target: " << TargetInfo::TargetType
                                                  << " ID: " << node.id() << node.name() << std::endl);

        BI_COMPUTE_ERROR_ON(TargetInfo::TargetType != node.assigned_target());
        BI_COMPUTE_ERROR_ON(node.num_inputs() != num_expected_inputs);
        BI_COMPUTE_ERROR_ON(node.num_outputs() != num_expected_outputs);
        BI_COMPUTE_UNUSED(node, num_expected_inputs, num_expected_outputs);
    }
    ...
    std::unique_ptr<BIIFunction> create_activation_layer(ActivationLayerNode &node)
    {
    ...
    std::unique_ptr<BIIFunction> create_arg_min_max_layer(ArgMinMaxLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
        ...
        typename TargetInfo::TensorType *input  = get_backing_tensor<TargetInfo>(node.input(0));
        typename TargetInfo::TensorType *output = get_backing_tensor<TargetInfo>(node.output(0));
        const BIReductionOperation       op     = node.reduction_operation();
        ...
        BI_COMPUTE_LOG_GRAPH_INFO("Instantiated " << node.name() << " Type: " << node.type() << " Target: "
    ...
    std::unique_ptr<BIIFunction> create_batch_normalization_layer(BatchNormalizationLayerNode &node)
    {
        ...
        const BIActivationLayerInfo      fused_act = node.fused_activation();
    ...
    std::unique_ptr<BIIFunction>
    create_fused_convolution_batch_normalization_layer(FusedConvolutionBatchNormalizationNode &node, BIGraphContext &ctx)
    {
        ...
        std::shared_ptr<BIIMemoryManager> mm = get_memory_manager(ctx, TargetInfo::TargetType);
        std::unique_ptr<BIIFunction>      func;
    ...
    std::unique_ptr<BIIFunction> create_bounding_box_transform_layer(BoundingBoxTransformLayerNode &node)
    {
    ...
    std::unique_ptr<BIIFunction> create_channel_shuffle_layer(ChannelShuffleLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BatmanInfer::BIIFunction> create_concatenate_layer(ConcatenateLayerNode &node)
    {
        BI_COMPUTE_LOG_GRAPH_VERBOSE("Creating Concatenate node with ID : " << node.id() << " and Name: " << node.name()
                                                                         << std::endl);
        BI_COMPUTE_ERROR_ON(node.num_outputs() != 1);
        ...
        typename TargetInfo::TensorType *output = get_backing_tensor<TargetInfo>(node.output(0));
        const BIDataLayout data_layout = node.output(0) != nullptr ? node.output(0)->desc().layout : BIDataLayout::UNKNOWN;
    ...
    std::unique_ptr<BIIFunction> create_convolution_layer(ConvolutionLayerNode &node, BIGraphContext &ctx)
    {
        ...
        if (is_quantized)
        {
            biases->info()->set_data_type(BIDataType::S32);
        }
        ...
        if (conv_algorithm == BIConvolutionMethod::Winograd)
        {
            BI_COMPUTE_ERROR_ON_MSG(num_groups != 1, "WinogradConvolutionLayer does not support grouping!");
        ...
        else if (conv_algorithm == BIConvolutionMethod::Direct)
        {
            BI_COMPUTE_ERROR_ON_MSG(num_groups != 1, "DirectConvolutionLayer does not support grouping!");
        ...
        else if (conv_algorithm == BIConvolutionMethod::GEMM)
        {
            std::tie(func, func_name) =
                create_named_memory_managed_function<typename ConvolutionLayerFunctions::GEMMConvolutionLayer>(
                    std::string("GEMMConvolutionLayer"), mm, input, weights, biases, output, conv_info, BIWeightsInfo(),
        ...
        else
        {
            std::tie(func, func_name) =
                create_named_memory_managed_function<typename ConvolutionLayerFunctions::GenericConvolutionLayer>(
                    std::string("GenericConvolutionLayer"), mm, input, weights, biases, output, conv_info, BIWeightsInfo(),
    ...
    std::unique_ptr<BIIFunction> create_deconvolution_layer(DeconvolutionLayerNode &node, BIGraphContext &ctx)
    {
        validate_node<TargetInfo>(node, 3 /* expected inputs */, 1 /* expected outputs */);
        ...
        const BIPadStrideInfo deconv_info = node.deconvolution_info();
    ...
    std::unique_ptr<BIIFunction> create_depthwise_convolution_layer(DepthwiseConvolutionLayerNode &node)
    {
        validate_node<TargetInfo>(node, 3 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_depth_to_space_layer(DepthToSpaceLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_dequantization_layer(DequantizationLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_detection_output_layer(DetectionOutputLayerNode &node)
    {
        validate_node<TargetInfo>(node, 3 /* expected inputs */, 1 /* expected outputs */);
        ...
        BI_COMPUTE_ERROR_ON(input1 == nullptr);
        BI_COMPUTE_ERROR_ON(input2 == nullptr);
        BI_COMPUTE_ERROR_ON(output == nullptr);
    ...
    std::unique_ptr<BIIFunction> create_detection_post_process_layer(DetectionPostProcessLayerNode &node)
    {
        ...
        BI_COMPUTE_ERROR_ON(input1 == nullptr);
        BI_COMPUTE_ERROR_ON(input2 == nullptr);
        BI_COMPUTE_ERROR_ON(output0 == nullptr);
        BI_COMPUTE_ERROR_ON(output1 == nullptr);
        BI_COMPUTE_ERROR_ON(output2 == nullptr);
        BI_COMPUTE_ERROR_ON(output3 == nullptr);
    ...
    std::unique_ptr<BIIFunction> create_eltwise_layer(EltwiseLayerNode &node)
    {
        validate_node<TargetInfo>(node, 2 /* expected inputs */, 1 /* expected outputs */);
        ...
        std::unique_ptr<BIIFunction> func = nullptr;
        std::string                func_name;
        ...
        // Log info
        BI_COMPUTE_LOG_GRAPH_INFO("Instantiated " << node.name() << " Type: " << node.type()
    ...
    std::unique_ptr<BIIFunction> create_unary_eltwise_layer(UnaryEltwiseLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_flatten_layer(FlattenLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_fully_connected_layer(FullyConnectedLayerNode &node, BIGraphContext &ctx)
    {
        validate_node<TargetInfo>(node, 3 /* expected inputs */, 1 /* expected outputs */);
        ...
        typename TargetInfo::TensorType *biases  = get_backing_tensor<TargetInfo>(node.input(2));
        typename TargetInfo::TensorType *output  = get_backing_tensor<TargetInfo>(node.output(0));
        BIFullyConnectedLayerInfo        fc_info = node.info();
        fc_info.enable_fast_math                 = (node.fast_math_hint() == BIFastMathHint::Enabled);

        BI_COMPUTE_ERROR_ON(input == nullptr);
        BI_COMPUTE_ERROR_ON(weights == nullptr);
        BI_COMPUTE_ERROR_ON(output == nullptr);
    ...
    std::unique_ptr<BIIFunction> create_generate_proposals_layer(GenerateProposalsLayerNode &node, BIGraphContext &ctx)
    {
        ...
        BI_COMPUTE_ERROR_ON(scores == nullptr);
        BI_COMPUTE_ERROR_ON(deltas == nullptr);
        BI_COMPUTE_ERROR_ON(anchors == nullptr);
        BI_COMPUTE_ERROR_ON(proposals == nullptr);
        BI_COMPUTE_ERROR_ON(scores_out == nullptr);
    ...
    std::unique_ptr<BIIFunction> create_l2_normalize_layer(L2NormalizeLayerNode &node, BIGraphContext &ctx)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_normalization_layer(NormalizationLayerNode &node, BIGraphContext &ctx)
    {
    ...
    std::unique_ptr<BIIFunction> create_normalize_planar_yuv_layer(NormalizePlanarYUVLayerNode &node)
    {
        validate_node<TargetInfo>(node, 3 /* expected inputs */, 1 /* expected outputs */);
        ...
        typename TargetInfo::TensorType *output = get_backing_tensor<TargetInfo>(node.output(0));
        BI_COMPUTE_ERROR_ON(input == nullptr);
        BI_COMPUTE_ERROR_ON(mean == nullptr);
        BI_COMPUTE_ERROR_ON(std == nullptr);
        BI_COMPUTE_ERROR_ON(output == nullptr);
    ...
    std::unique_ptr<BIIFunction> create_pad_layer(PadLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
        ...
        const BIPixelValue               pad_value = node.pad_value();
        BI_COMPUTE_ERROR_ON(input == nullptr);
        BI_COMPUTE_ERROR_ON(output == nullptr);
    ...
    std::unique_ptr<BIIFunction> create_permute_layer(PermuteLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_pooling_layer(PoolingLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
        ...
        typename TargetInfo::TensorType *input     = get_backing_tensor<TargetInfo>(node.input(0));
        typename TargetInfo::TensorType *output    = get_backing_tensor<TargetInfo>(node.output(0));
        const BIPoolingLayerInfo         pool_info = node.pooling_info();
        BI_COMPUTE_ERROR_ON(input == nullptr);
        BI_COMPUTE_ERROR_ON(output == nullptr);
    ...
    std::unique_ptr<BIIFunction> create_prelu_layer(PReluLayerNode &node)
    {
        validate_node<TargetInfo>(node, 2 /* expected inputs */, 1 /* expected outputs */);
        ...
        typename TargetInfo::TensorType *output = get_backing_tensor<TargetInfo>(node.output(0));
        BI_COMPUTE_ERROR_ON(input == nullptr || alpha == nullptr);
        BI_COMPUTE_ERROR_ON(output == nullptr);
     ...
     */
    template <typename TargetInfo>
    std::unique_ptr<BIIFunction> create_print_layer(PrintLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);

        typename TargetInfo::TensorType *input = get_backing_tensor<TargetInfo>(node.input(0));
        BI_COMPUTE_ERROR_ON(input == nullptr);
        BI_COMPUTE_UNUSED(input);
    ...
    std::unique_ptr<BIIFunction> create_priorbox_layer(PriorBoxLayerNode &node)
    {
        validate_node<TargetInfo>(node, 2 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_quantization_layer(QuantizationLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_reduction_operation_layer(ReductionLayerNode &node, BIGraphContext &ctx)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
        ...
        typename TargetInfo::TensorType *input     = get_backing_tensor<TargetInfo>(node.input(0));
        typename TargetInfo::TensorType *output    = get_backing_tensor<TargetInfo>(node.output(0));
        BIReductionOperation             op        = node.op();
        int                              axis      = node.axis();
    ...
    std::unique_ptr<BIIFunction> create_reorg_layer(ReorgLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_reshape_layer(ReshapeLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_resize_layer(ResizeLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
        ...
        func->configure(input, output,
                        ScaleKernelInfo{policy, BorderMode::CONSTANT, BIPixelValue(), SamplingPolicy::CENTER, false, false});
    ...
    std::unique_ptr<BIIFunction> create_roi_align_layer(ROIAlignLayerNode &node)
    {
        validate_node<TargetInfo>(node, 2 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_slice_layer(SliceLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_softmax_layer(SoftmaxLayerNode &node, BIGraphContext &ctx)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
    ...
    std::unique_ptr<BIIFunction> create_stack_layer(StackLayerNode &node)
    {
        BI_COMPUTE_LOG_GRAPH_VERBOSE("Creating Stack node with ID : " << node.id() << " and Name: " << node.name()
                                                                      << std::endl);
        BI_COMPUTE_ERROR_ON(node.num_outputs() != 1);
    ...
    std::unique_ptr<BIIFunction> create_strided_slice_layer(StridedSliceLayerNode &node)
    {
        validate_node<TargetInfo>(node, 1 /* expected inputs */, 1 /* expected outputs */);
        ...
        typename TargetInfo::TensorType *input   = get_backing_tensor<TargetInfo>(node.input(0));
        typename TargetInfo::TensorType *output  = get_backing_tensor<TargetInfo>(node.output(0));
        BICoordinates                    starts  = node.starts();
        BICoordinates                    ends    = node.ends();
        BiStrides                        strides = node.strides();
        BIStridedSliceLayerInfo          info    = node.strided_slice_info();
