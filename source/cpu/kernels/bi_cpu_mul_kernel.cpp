...
#include "cpu/kernels/mul/generic/neon/list.hpp"
    ...
    namespace cpu {
        namespace kernels {
            namespace {
                ...
                inline BIStatus validate_arguments(const BIITensorInfo *src1,
                                                   const BIITensorInfo *src2,
                                                   const BIITensorInfo *dst,
                                                   float scale,
                                                   BIConvertPolicy overflow_policy,
                                                   BIRoundingPolicy rounding_policy) {
                    BI_COMPUTE_UNUSED(overflow_policy);
                    BI_COMPUTE_UNUSED(rounding_policy);

                    BI_COMPUTE_RETURN_ERROR_ON_CPU_F16_UNSUPPORTED(src1);
                    BI_COMPUTE_RETURN_ERROR_ON_DATA_TYPE_CHANNEL_NOT_IN(src1, 1, BIDataType::U8, BIDataType::QASYMM8,
                                                                        BIDataType::QASYMM8_SIGNED, BIDataType::S16,
                                                                        BIDataType::S32,
                                                                        BIDataType::QSYMM16, BIDataType::F16,
                                                                        BIDataType::F32);
                    BI_COMPUTE_RETURN_ERROR_ON_DATA_TYPE_CHANNEL_NOT_IN(src2, 1, BIDataType::U8, BIDataType::QASYMM8,
                                                                        BIDataType::QASYMM8_SIGNED, BIDataType::S16,
                                                                        BIDataType::S32,
                                                                        BIDataType::QSYMM16, BIDataType::F16,
                                                                        BIDataType::F32);
                    BI_COMPUTE_RETURN_ERROR_ON_DATA_TYPE_CHANNEL_NOT_IN(dst, 1, BIDataType::U8, BIDataType::QASYMM8,
                                                                        BIDataType::QASYMM8_SIGNED, BIDataType::S16,
                                                                        BIDataType::QSYMM16,
                                                                        BIDataType::S32, BIDataType::F16,
                                                                        BIDataType::F32);
                    if (is_data_type_quantized(src1->data_type()) || is_data_type_quantized(src2->data_type())) {
                        BI_COMPUTE_RETURN_ERROR_ON_MISMATCHING_DATA_TYPES(src1, src2);
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(overflow_policy == BIConvertPolicy::WRAP,
                                                       "ConvertPolicy cannot be WRAP if datatype is quantized");
                    }

                    if (dst->total_size() > 0) {
                        const BITensorShape &out_shape = BITensorShape::broadcast_shape(src1->tensor_shape(),
                                                                                        src2->tensor_shape());
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(
                                detail::have_different_dimensions(out_shape, dst->tensor_shape(), 0),
                                "Wrong shape for dst");
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(out_shape.total_size() == 0,
                                                       "Inputs are not broadcast compatible");
                        // clang-format off
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(
                                !(src1->data_type() == src2->data_type() && src2->data_type() == dst->data_type()) &&
                                !(src1->data_type() == BIDataType::U8 && src2->data_type() == BIDataType::U8 &&
                                  dst->data_type() == BIDataType::S16) &&
                                !(src1->data_type() == BIDataType::U8 && src2->data_type() == BIDataType::S16 &&
                                  dst->data_type() == BIDataType::S16) &&
                                !(src1->data_type() == BIDataType::S16 && src2->data_type() == BIDataType::U8 &&
                                  dst->data_type() == BIDataType::S16) &&
                                !(src1->data_type() == BIDataType::S16 && src2->data_type() == BIDataType::U8 &&
                                  dst->data_type() == BIDataType::S16) &&
                                !(src1->data_type() == BIDataType::QSYMM16 &&
                                  src2->data_type() == BIDataType::QSYMM16 &&
                                  dst->data_type() == BIDataType::S32), "Invalid data type combination");
                        // clang-format on
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(
                                src1->data_type() == BIDataType::S16 && dst->data_type() == BIDataType::S32 &&
                                scale != 1.f,
                                "Unsupported scale for QSYMM16 inputs and S32 dst");
                    }

                    if (std::abs(scale - scale255_constant) < 0.00001f) {
                        BI_COMPUTE_RETURN_ERROR_ON(rounding_policy != BIRoundingPolicy::TO_NEAREST_UP &&
                                                   rounding_policy != BIRoundingPolicy::TO_NEAREST_EVEN);
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(
                                src1->data_type() == BIDataType::S32 && src2->data_type() == BIDataType::S32 &&
                                dst->data_type() == BIDataType::S32,
                                "Scale == 1/255 is not supported if input and dst are of data type S32");
                    } else {
                        BI_COMPUTE_RETURN_ERROR_ON(rounding_policy != BIRoundingPolicy::TO_ZERO);
                    ...
                    if (broadcast_x) {
                        const BIDataType dtype1 = src1->data_type();
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(dtype1 == BIDataType::QSYMM16 || dtype1 == BIDataType::U8 ||
                                                       dtype1 == BIDataType::S16,
 ...
 */
                inline int32x4_t scale255_S32_S32(int32x4_t in) {
                    ...
                    // Afterward vcvt rounds toward zero
                ...
                vquantize(float32x4x4_t val, const BIUniformQuantizationInfo &info) {
                ...
                template<typename T>
                void mul_saturate_quantized_8(const BIITensor *src1, const BIITensor *src2, BIITensor *out,
                    ...
                    const bool is_broadcast_across_x =
                            src1->info()->tensor_shape().x() != src2->info()->tensor_shape().x();

                    const BIUniformQuantizationInfo output_qua_info = out->info()->quantization_info().uniform();
                    const BIUniformQuantizationInfo tmp_qua_info = {output_qua_info.scale / scale,
                                                                    output_qua_info.offset};
                    ...
                    if (is_broadcast_across_x) {
                        ...
                        execute_window_loop(
                                ...
                                [&](const BICoordinates &) {
                                    ...
                                    for (; x <= (window_end_x - window_step_x); x += window_step_x) {
                                        ...
                                        // Dequantize inputs
                                        const float32x4x4_t in1_f32x4x4 = vdequantize(non_broadcast_v,
                                                                                      non_broadcast_qinfo);
                                        const float32x4x4_t in2_f32x4x4 = vdequantize(broadcast_value_vec,
                                                                                      broadcast_qinfo);
                                    ...
                                    for (; x < window_end_x; ++x) {
                                        ...
                                        const float tmp_in1 = BIQasymm8QuantizationHelper<T>::dequantize(src1,
                                                                                                         non_broadcast_qinfo);
                                        const float tmp_in2 = BIQasymm8QuantizationHelper<T>::dequantize(
                                                broadcast_value,
                                                broadcast_qinfo);
                                        const float tmp_f = tmp_in1 * tmp_in2;

                                        // Quantize dst
                                        const auto tmp_qua = BIQasymm8QuantizationHelper<T>::quantize(tmp_f,
                                                                                                      tmp_qua_info);
                        ...
                        const BIUniformQuantizationInfo input1_qua_info = src1->info()->quantization_info().uniform();
                        const BIUniformQuantizationInfo input2_qua_info = src2->info()->quantization_info().uniform();
                        ...
                        execute_window_loop(
                                ...
                                [&](const BICoordinates &) {
                                    ...
                                    for (; x < window_end_x; ++x) {
                                        ...
                                        const float tmp_in1 = BIQasymm8QuantizationHelper<T>::dequantize(src1,
                                                                                                         input1_qua_info);
                                        const float tmp_in2 = BIQasymm8QuantizationHelper<T>::dequantize(src2,
                                                                                                         input2_qua_info);
                                        const float tmp_f = tmp_in1 * tmp_in2;
                ...
                bool
                mul_q8_sme_possible(const BIITensorInfo *src0, const BIITensorInfo *src1, const BIITensorInfo *dst,
                                    float scale) {
                    ...
                    if (in0_shape.x() == in1_shape.x() && CPUInfo::get().has_sme2() &&
                        (in0_shape == in1_shape || dst_dims <= 4)) {
                ...
                bool mul_q8_neon_fixedpoint_possible(const BIITensorInfo *src0,
                ...
                void mul_q8_neon_fixedpoint(const BIITensor *src0, const BIITensor *src1, BIITensor *dst,
                                            const BIWindow &window,
                                            float scale) {
                    ...
                    // Create input windows.
                    BIWindow in0_win = window.broadcast_if_dimension_le_one(in0_shape);
                    BIWindow in1_win = window.broadcast_if_dimension_le_one(in1_shape);
        ...
        const auto b_offset = is_broadcast_input_1 ? in1_offset : in0_offset;
                        ...
                        // Clear the x dimension on the execution window as we process the whole row each iteration.
                        a_win.set(BIWindow::DimX, BIWindow::BIDimension(0, 1, 1));

                        BIIterator a_input_it(a_tensor, a_win);
                        BIIterator b_input_it(b_tensor, b_win);
                        BIIterator out_it(dst, win);
                        ...
                        execute_window_loop(
                                ...
                                [&](const BICoordinates &) {
                                    ...
                                    const auto b_voffseted_32p0 = wrapper::vdup_n(b_offseted_32p0,
                                                                                  wrapper::traits::vector_128_tag());

                                    const auto vmultiplier_14p18 = wrapper::vdup_n(multiplier_14p18,
                                                                                   wrapper::traits::vector_128_tag());
                                    const auto voffsetout_14p18 = wrapper::vdup_n(out_offset_14p18,
                                                                                  wrapper::traits::vector_128_tag());
                                    ...
                                    for (; x <= (window_end_x - window_step_x); x += window_step_x) {
                                        ...
                                        const auto a_vin_16p0_0 = wrapper::vreinterpret(
                                                wrapper::vmovl(wrapper::vgetlow(a_vin_8p0)));
                                        const auto a_vin_16p0_1 = wrapper::vreinterpret(
                                                wrapper::vmovl(wrapper::vgethigh(a_vin_8p0)));

                                        const auto voffseted_32p0_00 = wrapper::vsubl(wrapper::vgetlow(a_vin_16p0_0),
                                                                                      a_voffset_16p0);
                                        const auto voffseted_32p0_01 = wrapper::vsubl(wrapper::vgethigh(a_vin_16p0_0),
                                                                                      a_voffset_16p0);
                                        const auto voffseted_32p0_10 = wrapper::vsubl(wrapper::vgetlow(a_vin_16p0_1),
                                                                                      a_voffset_16p0);
                                        const auto voffseted_32p0_11 = wrapper::vsubl(wrapper::vgethigh(a_vin_16p0_1),
                                                                                      a_voffset_16p0);

                                        const auto vinnermul_32p0_00 = wrapper::vmul(voffseted_32p0_00,
                                                                                     b_voffseted_32p0);
                                        const auto vinnermul_32p0_01 = wrapper::vmul(voffseted_32p0_01,
                                                                                     b_voffseted_32p0);
                                        const auto vinnermul_32p0_10 = wrapper::vmul(voffseted_32p0_10,
                                                                                     b_voffseted_32p0);
                                        const auto vinnermul_32p0_11 = wrapper::vmul(voffseted_32p0_11,
                                                                                     b_voffseted_32p0);

                                        const auto vout_14p18_00 = wrapper::vmla(voffsetout_14p18, vinnermul_32p0_00,
                                                                                 vmultiplier_14p18);
                                        const auto vout_14p18_01 = wrapper::vmla(voffsetout_14p18, vinnermul_32p0_01,
                                                                                 vmultiplier_14p18);
                                        const auto vout_14p18_10 = wrapper::vmla(voffsetout_14p18, vinnermul_32p0_10,
                                                                                 vmultiplier_14p18);
                                        const auto vout_14p18_11 = wrapper::vmla(voffsetout_14p18, vinnermul_32p0_11,
                                                                                 vmultiplier_14p18);
                                        ...
                                        const auto vout_15p1_00 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_00));
                                        const auto vout_15p1_01 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_01));
                                        const auto vout_15p1_10 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_10));
                                        const auto vout_15p1_11 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_11));
                                        ...
                                        const auto vout_8p0 =
                                                wrapper::vcombine(wrapper::vqrshrn<2>(vout_15p1_0),
                                    ...
                                    for (; x < window_end_x; ++x) {
#ifdef __aarch64__
                                        out_ptr[x] = wrapper::vqrshrn<2>(
                                                wrapper::vqrshrn_ex<8, ScalarType>(wrapper::vshrq_n<8>(
                                                        (multiplier_14p18 * (int32_t(a_ptr[x]) - a_offset_16p0) *
                                                         (int32_t(b_val) - b_offset_16p0)) +
                                                        out_offset_14p18)));
#else  //__aarch64__
                        ...
                        // Clear the x dimension on the execution window as we process the whole row each iteration.
                        in0_win.set(BIWindow::DimX, BIWindow::BIDimension(0, 1, 1));
                        in1_win.set(BIWindow::DimX, BIWindow::BIDimension(0, 1, 1));

                        BIIterator in0_it(src0, in0_win);
                        BIIterator in1_it(src1, in1_win);
                        BIIterator out_it(dst, win);
                        ...
                        execute_window_loop(
                                ...
                                [&](const BICoordinates &) {
                                    ...
                                    for (; x <= (window_end_x - window_step_x); x += window_step_x) {
                                        ...
                                        const auto vin0_16p0_0 = wrapper::vreinterpret(
                                                wrapper::vmovl(wrapper::vgetlow(vin0_8p0)));
                                        const auto vin0_16p0_1 = wrapper::vreinterpret(
                                                wrapper::vmovl(wrapper::vgethigh(vin0_8p0)));
                                        const auto vin1_16p0_0 = wrapper::vreinterpret(
                                                wrapper::vmovl(wrapper::vgetlow(vin1_8p0)));
                                        const auto vin1_16p0_1 = wrapper::vreinterpret(
                                                wrapper::vmovl(wrapper::vgethigh(vin1_8p0)));

                                        const auto voffseted0_32p0_00 = wrapper::vsubl(wrapper::vgetlow(vin0_16p0_0),
                                                                                       voffset0_16p0);
                                        const auto voffseted0_32p0_01 = wrapper::vsubl(wrapper::vgethigh(vin0_16p0_0),
                                                                                       voffset0_16p0);
                                        const auto voffseted0_32p0_10 = wrapper::vsubl(wrapper::vgetlow(vin0_16p0_1),
                                                                                       voffset0_16p0);
                                        const auto voffseted0_32p0_11 = wrapper::vsubl(wrapper::vgethigh(vin0_16p0_1),
                                                                                       voffset0_16p0);

                                        const auto voffseted1_32p0_00 = wrapper::vsubl(wrapper::vgetlow(vin1_16p0_0),
                                                                                       voffset1_16p0);
                                        const auto voffseted1_32p0_01 = wrapper::vsubl(wrapper::vgethigh(vin1_16p0_0),
                                                                                       voffset1_16p0);
                                        const auto voffseted1_32p0_10 = wrapper::vsubl(wrapper::vgetlow(vin1_16p0_1),
                                                                                       voffset1_16p0);
                                        const auto voffseted1_32p0_11 = wrapper::vsubl(wrapper::vgethigh(vin1_16p0_1),
                                                                                       voffset1_16p0);

                                        const auto vinnermul_32p0_00 = wrapper::vmul(voffseted0_32p0_00,
                                                                                     voffseted1_32p0_00);
                                        const auto vinnermul_32p0_01 = wrapper::vmul(voffseted0_32p0_01,
                                                                                     voffseted1_32p0_01);
                                        const auto vinnermul_32p0_10 = wrapper::vmul(voffseted0_32p0_10,
                                                                                     voffseted1_32p0_10);
                                        const auto vinnermul_32p0_11 = wrapper::vmul(voffseted0_32p0_11,
                                                                                     voffseted1_32p0_11);
                                        ...
                                        // These shift rights are to revert the multiplication by twopwr18. Hard limit of a maximum shift by 8 requires multiple shift instructions to achieve this.
                                        const auto vout_14p2_00 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_00));
                                        const auto vout_14p2_01 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_01));
                                        const auto vout_14p2_10 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_10));
                                        const auto vout_14p2_11 = wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(vout_14p18_11));
                                        ...
                                        const auto vout_8p0 =
                                                wrapper::vcombine(wrapper::vqrshrn<2>(vout_14p2_0),
                                    ...
                                    for (; x < window_end_x; ++x) {
                                        ...
                                        out_ptr[x] = wrapper::vqrshrn<2>(wrapper::vqrshrn_ex<8, ScalarType>(
                                                wrapper::vshrq_n<8>(
                                                        (multiplier_14p18 * (int32_t(in0_ptr[x]) - in0_offset_16p0) *
                ...
                void mul_saturate_QSYMM16_QSYMM16_QSYMM16(
                        const BIITensor *src1, const BIITensor *src2, BIITensor *out, const BIWindow &window,
                        float scale) {
                    const BIUniformQuantizationInfo input1_qua_info = src1->info()->quantization_info().uniform();
                    const BIUniformQuantizationInfo input2_qua_info = src2->info()->quantization_info().uniform();
                    const BIUniformQuantizationInfo output_qua_info = out->info()->quantization_info().uniform();
                    ...
                    execute_window_loop(
                            ...
                            [&](const BICoordinates &) {
                                ...
                                for (; x < window_end_x; ++x) {
                                    ...
                                    qsymm16_t tmp_qua =
                                            static_cast<qsymm16_t>(tmp > SHRT_MAX) ? SHRT_MAX : ((tmp < SHRT_MIN)
                                                                                                 ? SHRT_MIN
                                                                                                 : tmp);
                                    *(output_ptr + x) = tmp_qua;
                                }
                ...
                void mul_QSYMM16_QSYMM16_S32(const BIITensor *src1, const BIITensor *src2, BIITensor *out,
                                             const BIWindow &window,
                                             int scale) {
                    BI_COMPUTE_UNUSED(scale);
                    ...
                    execute_window_loop(
                            ...
                            [&](const BICoordinates &) {
                                ...
                                // Compute left-over elements
                                for (; x < window_end_x; ++x) {
                                    int32_t tmp = static_cast<int32_t>(*(input1_ptr + x)) *
                                                  static_cast<int32_t>(*(input2_ptr + x));
                                    *(output_ptr + x) = tmp;
                                }
                            },
                            input1, input2, dst);
                ...
                void mul_U8_U8_U8(const BIITensor *src1, const BIITensor *src2, BIITensor *out, const BIWindow &window,
                                  int n) {
                    // Create input windows
                    BIWindow win = window;
                    BIWindow input1_win = window.broadcast_if_dimension_le_one(src1->info()->tensor_shape());
                    ...
                    execute_window_loop(
                            ...
                            [&](const BICoordinates &) {
                                ...
                                for (; x <= (window_end_x - window_step_x); x += window_step_x) {
                                    ...
                                    if (is_sat) {
                                        vst1q_u8(output_ptr + x,
                                                 vcombine_u8(vqmovn_u16(tmp1_low), vqmovn_u16(tmp1_high)));
                                    } else {
                                        vst1q_u8(output_ptr + x,
                                                 vcombine_u8(vmovn_u16(tmp1_low), vmovn_u16(tmp1_high)));
                                    }
                                }

                                // Compute left-over elements
                                for (; x < window_end_x; ++x) {
                                    uint16_t tmp = static_cast<uint16_t>(*(input1_ptr + x)) *
                                                   static_cast<uint16_t>(*(input2_ptr + x));
                ...
                inline int16x8x2_t mul_S16_S16_S16_n_k(const int16x8x2_t &src1, const int16x8x2_t &src2, int n) {
                    const int16x8x2_t result = {{// First 8 elements
                                                        mul_S16_S16_S16_n_loop<is_scale255, is_sat>(src1.val[0],
                                                                                                    src2.val[0], n),
                                                        // Second 8 elements
                                                        mul_S16_S16_S16_n_loop<is_scale255, is_sat>(src1.val[1],
                                                                                                    src2.val[1], n)}};
                ...
                template<bool is_scale255, bool is_sat>
                void
                mul_S16_S16_S16(const BIITensor *src1, const BIITensor *src2, BIITensor *out, const BIWindow &window,
                                int n) {
                    // Create input windows
                    BIWindow win = window;
                    BIWindow input1_win = window.broadcast_if_dimension_le_one(src1->info()->tensor_shape());
                ...
                template<bool is_sat>
                void
                mul_S32_S32_S32(const BIITensor *src1, const BIITensor *src2, BIITensor *out, const BIWindow &window,
                                int n) {
                    // Create input windows
                    BIWindow input1_win = window.broadcast_if_dimension_le_one(src1->info()->tensor_shape());
                    BIWindow input2_win = window.broadcast_if_dimension_le_one(src2->info()->tensor_shape());
                    ...
                    if (is_broadcast_across_x) {
                                ...
                                [&](const BICoordinates &) {
                                    ...
                                    for (; x <= (window_end_x - window_step_x); x += window_step_x) {
                                        ...
                                        const int32x4x2_t non_broadcast_v = {{
                                                                                     vld1q_s32(non_broadcast_input_ptr +
                                                                                               x),
                                                                                     vld1q_s32(non_broadcast_input_ptr +
                                                                                               x +
                                                                                               4),
                                                                             }};
                                        const int32x4x2_t result = mul_S32_S32_S32_n_k<is_sat>(broadcast_v,
                                                                                               non_broadcast_v,
                                                                                               n);
                                    ...
                                    for (; x < window_end_x; ++x) {
                                        int64_t tmp =
                                                static_cast<int64_t>(broadcast_value) *
                                                static_cast<int64_t>(*(non_broadcast_input_ptr + x));
                                        ...
                                        if (is_sat) {
                                            tmp = misc::utility::clamp<int64_t, int32_t>(tmp);
                                        }
                        ...
                        execute_window_loop(
                                ...
                                [&](const BICoordinates &) {
                                    ...
                                    // Compute left-over elements
                                    for (; x < window_end_x; ++x) {
                                        int64_t tmp = static_cast<int64_t>(*(input1_ptr + x)) *
                                                      static_cast<int64_t>(*(input2_ptr + x));
                ...
                void c_mul_F32_F32_F32_n(const BIITensor *src1, const BIITensor *src2, BIITensor *out,
                                         const BIWindow &window) {
                    // Create input windows
                    BIWindow input1_win = window.broadcast_if_dimension_le_one(src1->info()->tensor_shape());
                    BIWindow input2_win = window.broadcast_if_dimension_le_one(src2->info()->tensor_shape());
                        ...
                        execute_window_loop(
                                ...
                                [&](const BICoordinates &) {
                                    ...
                                    for (; x <= (window_end_x - window_step_x); x += window_step_x) {
                                        ...
                                        const float32x2_t tmp00 = wrapper::vdup_n(wrapper::vgetlane(a, 0),
                                                                                  ExactTagType{});
                                        const float32x2_t tmp01 = wrapper::vdup_n(wrapper::vgetlane(a, 1),
                                                                                  ExactTagType{});
                                        const float32x2_t tmp10 = wrapper::vdup_n(wrapper::vgetlane(a, 2),
                                                                                  ExactTagType{});
                                        const float32x2_t tmp11 = wrapper::vdup_n(wrapper::vgetlane(a, 3),
                                                                                  ExactTagType{});
                ...
                template<bool is_scale255, bool is_sat>
                void mul_U8_U8_S16(const BIITensor *src1, const BIITensor *src2, BIITensor *out, const BIWindow &window,
                                   int n) {
                    // Create input windows
                    BIWindow win = window;
                    BIWindow input1_win = window.broadcast_if_dimension_le_one(src1->info()->tensor_shape());
                ...
                template<bool is_scale255, bool is_sat>
                void
                mul_S16_U8_S16(const BIITensor *src1, const BIITensor *src2, BIITensor *out, const BIWindow &window,
                               int n) {
                    // Create input windows
                    BIWindow win = window;
                    BIWindow input1_win = window.broadcast_if_dimension_le_one(src1->info()->tensor_shape());
                ...
                template<bool is_scale255, bool is_sat>
                void
                mul_U8_S16_S16(const BIITensor *src1, const BIITensor *src2, BIITensor *out, const BIWindow &window,
                               int n) {
            ...
            void BICpuMulKernel::configure(BIITensorInfo *src1,
                                           BIITensorInfo *src2,
                                           BIITensorInfo *dst,
                                           float scale,
                                           BIConvertPolicy overflow_policy,
                                           BIRoundingPolicy rounding_policy) {
                BI_COMPUTE_UNUSED(rounding_policy);
                BI_COMPUTE_ERROR_ON_NULLPTR(src1, src2, dst);

                BI_COMPUTE_ERROR_THROW_ON(validate_arguments(src1, src2, dst, scale, overflow_policy, rounding_policy));
                ...
                const BIDataType dt_input1 = src1->data_type();
                const BIDataType dt_input2 = src2->data_type();
                const BIDataType dt_output = dst->data_type();
                const bool is_sat = (overflow_policy == BIConvertPolicy::SATURATE);

                switch (dt_input1) {
                    case BIDataType::QASYMM8:
                        if (dt_input2 == BIDataType::QASYMM8 && dt_output == BIDataType::QASYMM8) {
                    ...
                    case BIDataType::QASYMM8_SIGNED:
                        if (dt_input2 == BIDataType::QASYMM8_SIGNED) {
                            if (mul_q8_sme_possible(src1, src2, dst, scale) &&
                                rounding_policy == BIRoundingPolicy::TO_ZERO) {
                    ...
                    case BIDataType::QSYMM16:
                        if (dt_input2 == BIDataType::QSYMM16 && dt_output == BIDataType::QSYMM16) {
                            _func_quantized = &mul_saturate_QSYMM16_QSYMM16_QSYMM16;
                        } else if (dt_input2 == BIDataType::QSYMM16 && dt_output == BIDataType::S32) {
                            _func_int = &mul_QSYMM16_QSYMM16_S32;
                        }
                        break;
                    case BIDataType::S16:
                        if (BIDataType::U8 == dt_input2 && BIDataType::S16 == dt_output) {
                            if (is_scale_255) {
                        ...
                        if (BIDataType::S16 == dt_input2 && BIDataType::S16 == dt_output) {
                            if (is_scale_255) {
                        ...
                        break;
                    case BIDataType::S32:
                        if (BIDataType::S32 == dt_input2 && BIDataType::S32 == dt_output) {
                            _func_int = is_sat ? &mul_S32_S32_S32<true> : &mul_S32_S32_S32<false>;
                        }
                        break;
                    case BIDataType::U8:
                        if (BIDataType::U8 == dt_input2 && BIDataType::U8 == dt_output) {
                            if (is_scale_255) {
                        ...
                        } else if (BIDataType::U8 == dt_input2 && BIDataType::S16 == dt_output) {
                            if (is_scale_255) {
                        ...
                        } else if (BIDataType::S16 == dt_input2 && BIDataType::S16 == dt_output) {
                            if (is_scale_255) {
                    ...
                    case BIDataType::F32:
                        ...
                        break;
                    default:
                        BI_COMPUTE_ERROR("You called with the wrong img formats");
                }
            ...
            size_t BICpuMulKernel::get_mws(const CPUInfo &platform, size_t thread_count) const {
                ...
                if (this->_func_float == &mul_F32_F32_F32) {
                    ...
                    if (platform.get_cpu_model() == CPUModel::N1) {
                        ...
                        if (_split_dimension == BIWindow::DimX) {
            ...
            BIStatus BICpuMulKernel::validate(const BIITensorInfo *src1,
                                              const BIITensorInfo *src2,
                                              const BIITensorInfo *dst,
                                              float scale,
                                              BIConvertPolicy overflow_policy,
                ...
                BI_COMPUTE_RETURN_ON_ERROR(
                        validate_arguments(src1, src2, dst, scale, overflow_policy, rounding_policy));

                return BIStatus{};
            }

            void BICpuMulKernel::run_op(BIITensorPack &tensors, const BIWindow &window, const ThreadInfo &info) {
                ...
                auto src1 = tensors.get_const_tensor(BITensorType::ACL_SRC_0);
                auto src2 = tensors.get_const_tensor(BITensorType::ACL_SRC_1);
                ...
                if (_func_quantized != nullptr) {
                    ...
                    BI_COMPUTE_ERROR_ON(_func_float == nullptr);
                    (*_func_float)(src1, src2, dst, window, _scale);
                }
            }

            const char *BICpuMulKernel::name() const {
                return "BICpuMulKernel";
            }

            namespace {
                BIStatus
                validate_arguments_complex(const BIITensorInfo *src1,
                                           const BIITensorInfo *src2,
                                           const BIITensorInfo *dst) {
                    BI_COMPUTE_RETURN_ERROR_ON_DATA_TYPE_CHANNEL_NOT_IN(src1, 2, BIDataType::F32);
                    BI_COMPUTE_RETURN_ERROR_ON_DATA_TYPE_CHANNEL_NOT_IN(src2, 2, BIDataType::F32);
                    ...
                    if (dst->total_size() > 0) {
                        BI_COMPUTE_RETURN_ERROR_ON_DATA_TYPE_CHANNEL_NOT_IN(dst, 2, BIDataType::F32);
                        BI_COMPUTE_RETURN_ERROR_ON_MSG(
                                detail::have_different_dimensions(out_shape, dst->tensor_shape(), 0),
                                "Wrong shape for dst");
                    }
            ...
            void BICpuComplexMulKernel::configure(BIITensorInfo *src1, BIITensorInfo *src2, BIITensorInfo *dst) {
                BI_COMPUTE_ERROR_ON_NULLPTR(src1, src2, dst);
                BI_COMPUTE_ERROR_THROW_ON(validate_arguments_complex(src1, src2, dst));

                const BITensorShape &out_shape = BITensorShape::broadcast_shape(src1->tensor_shape(),
                                                                                src2->tensor_shape());

                // Auto initialize dst if not initialized
                const BITensorInfo out_info(out_shape, src1->num_channels(), src1->data_type());
                auto_init_if_empty(*dst, out_info);

                // Configure kernel window
                BIWindow win = calculate_max_window(out_shape);

                BIICpuKernel::configure(win);
            }

            BIStatus
            BICpuComplexMulKernel::validate(const BIITensorInfo *src1, const BIITensorInfo *src2,
                                            const BIITensorInfo *dst) {
                BI_COMPUTE_ERROR_ON_NULLPTR(src1, src2, dst);
                BI_COMPUTE_RETURN_ON_ERROR(validate_arguments_complex(src1, src2, dst));

                return BIStatus{};
            }

            void BICpuComplexMulKernel::run_op(BIITensorPack &tensors, const BIWindow &window, const ThreadInfo &info) {
                BI_COMPUTE_UNUSED(info);
                BI_COMPUTE_ERROR_ON_UNCONFIGURED_KERNEL(this);
                BI_COMPUTE_ERROR_ON_INVALID_SUBWINDOW(BIICpuKernel::window(), window);
            ...
            const char *BICpuComplexMulKernel::name() const {
                return "BICpuComplexMulKernel";
            }
        } // namespace kernels
    } // namespace cpu
} // namespace BatmanInfer