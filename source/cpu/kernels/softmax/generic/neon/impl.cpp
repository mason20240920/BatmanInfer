...
namespace BatmanInfer {
    namespace cpu {
        ...
        void neon_softmax_x_quantized(
            ...
            const BIUniformQuantizationInfo out_qinfo = out->info()->quantization_info().uniform();
                        ...
                        /* Compute exponentials and sum */
                        {
                            ...
                            for (; x <= (input_width - vec_size); x += vec_size) {
                                ...
                                if (IS_LOG) {
                                    ...
                                    vec_elements_flt.val[0] = vexpq_f32(
                                            vmulq_f32(vec_elements_flt.val[0], scale_beta_vec));
                                    vec_elements_flt.val[1] = vexpq_f32(
                                            vmulq_f32(vec_elements_flt.val[1], scale_beta_vec));
                                    vec_elements_flt.val[2] = vexpq_f32(
                                            vmulq_f32(vec_elements_flt.val[2], scale_beta_vec));
                                    vec_elements_flt.val[3] = vexpq_f32(
                                            vmulq_f32(vec_elements_flt.val[3], scale_beta_vec));
                            ...
                            const float32x4_t sum_16_byte =
                                    vaddq_f32(vaddq_f32(vec_sum.val[0], vec_sum.val[1]),
                        ...
                        /* Normalize exponentials */
                        {
                            ...
                            for (; x <= (input_width - vec_size); x += vec_size) {
                                ...
                                if (IS_LOG) {
                                    ...
                                    normalized_value = convert_float_to_int<float32x4x4_t, int_vec_type>(
                                            normalized_value_f);
                                } else {
                                    float32x4x4_t mul = {
                                    ...
                                    if (is_qasymm8_signed) {
                                        const auto offset_vec = wrapper::vdup_n(128.f,
...
#ifdef __aarch64__
                                    constexpr auto policy = BIRoundingPolicy::TO_NEAREST_EVEN;
#else  // __aarch64__
                                    constexpr auto policy = BIRoundingPolicy::TO_ZERO;
#endif // __aarch64__
                                    out_ptr[x] = BIQasymm8QuantizationHelper<T>::quantize(diff, out_qinfo, policy);
                                } else {
                    ...
                    [&](const BICoordinates &winCoords) {
                        ...
                        /* Compute Max */
                        {
                            if (!vector_exceeds_bounds) {
                                ...
                                int i = 0;
                                for (; i < axis_width; ++i) {
                                    const T *const base_ptr_in = ((i * in_axis_stride) +
                            ...
                            if (!vector_exceeds_bounds) {
                                ...
                                for (; i < axis_width; ++i) {
                                    vec_elements = wrapper::vloadq(
                                            (i * in_axis_stride) + reinterpret_cast<const T *>(in_ptr));
                                    vec_elements = wrapper::vqsub(vec_max, vec_elements);
                                    ...
                                    vst4q_f32((i * tmp_axis_stride) + reinterpret_cast<float *>(tmp_ptr),
                                              vec_elements_flt);
                                }
                                ...
                                for (; i < axis_width; ++i) {
                                    ...
                                    for (int j = 0; j < num_remaining_partial; ++j) {
                                        vec_els[num_remaining_full * 4 + j] = *(base_ptr_in +
                                                                                (4 * num_remaining_full + j));
                                    }
                                    for (int q = 0; q < 16; q++) {
                                    ...
                                    float *const base_ptr_tmp =
                                            (i * tmp_axis_stride) + reinterpret_cast<float *>(tmp_ptr);
                                    for (int k = 0; k < num_remaining_full; ++k) {
                                        for (int j = 0; j < 4; ++j) {
                        ...
                        /* Normalize exponentials */
                        {
                            ...
                            if (!vector_exceeds_bounds) {
                                ...
                                for (; i < axis_width; ++i) {
                                    using int_vec_type = wrapper::traits::neon_vector_t<T, 16>;
                                    float32x4x4_t vec_in = vld4q_f32(
                                            (i * tmp_axis_stride) + reinterpret_cast<float *>(tmp_ptr));

                                    int_vec_type normalized_value{};

                                    if (IS_LOG) {
                                        const float32x4_t out_offset = vdupq_n_f32(
                                                static_cast<float>(out_qinfo.offset));
                                        const float32x4_t out_inv_scale = vdupq_n_f32(1.f / out_qinfo.scale);

                                        const float32x4x4_t normalized_value_f = {
                                                vmlaq_f32(out_offset,
                                    ...
                                    wrapper::vstore((i * out_axis_stride) + reinterpret_cast<T *>(out_ptr),
                                                    normalized_value);
                                }
                            } else {
                                int i = 0;
                                    ...
                                    if (IS_LOG) {
                                        ...
                                        for (int k = 0; k < num_remaining_full; ++k) {
                                            for (int j = 0; j < 4; ++j) {
                                                ...
                                                *(base_ptr_out + (4 * k + j)) =
                                                        BIQasymm8QuantizationHelper<T>::quantize(diff, out_qinfo,
                                                                                                 policy);
                                            }
                                        }
                                        for (int j = 0; j < num_remaining_partial; ++j) {
                                            ...
                                            *(base_ptr_out + (4 * num_remaining_full + j)) =
                                                    BIQasymm8QuantizationHelper<T>::quantize(diff, out_qinfo, policy);
                                        }
                                    } else {
                                        for (int k = 0; k < num_remaining_full; ++k) {
                                            for (int j = 0; j < 4; ++j) {
