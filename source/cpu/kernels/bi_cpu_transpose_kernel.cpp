                            ...
                            return 4;
                                ...
                                void transpose_8bit_elements(const BIITensor *in,
                                        ...
                                        const int window_end_y_multiple_of =
                                                        ((window_end_y - window_start_y) / window_step_y) *
                                                        window_step_y;
                                        ...
                                        bool left_over_loop_y = (
                                                ((window_end_y - window_start_y) % window_step_y) != 0);
                                                ...
                                                if (window_end_y_multiple_of > window_step_y)
                                                        window_in.set(BIWindow::DimY,
                                                                      BIWindow::BIDimension(
                                                                              window_start_y, window_end_y_multiple_of,
                                                                              window_step_y));
                                                else
                                        ...
                                        if (in->info()->dimension(1) != 1) {
                                                ...
                                                execute_window_loop(
                                                        ...
                                                        [&](const BICoordinates &id) {
                                                                ...
                                                                for (; x <= (window_end_x - window_step_x);
                                                                       x += window_step_x) {
                                                                        ...
                                                                        vst1_u8(
                                                                                ...
                                                                                vreinterpret_u8_u16(vreinterpret_u16_u32
                                                                                        (k0_u32.val[0])));
                                                                        vst1_u8(
                                                                                reinterpret_cast<uint8_t *>(output.ptr()
                                                                                        + dst_offset_in_bytes +
                                                                ...
                                                                for (; x < window_end_x; ++x) {
                                                                        ...
                                                                        vst1_u8(output.ptr() + dst_offset_in_bytes,
                                                                                result);
                                                                }
                                                        }, input, output);
                                        }
                                        ...
                                        if (left_over_loop_y) {
                                                window_in.set(BIWindow::DimX,
                                                              BIWindow::BIDimension(
                                                                      window.x().start(), window.x().end(), 1));
                                                window_in.set(BIWindow::DimY,
                                                              BIWindow::BIDimension(
                                                                      window_end_y_multiple_of, window_end_y, 1));
                                ...
                                void transpose_16bit_elements(const BIITensor *in, BIITensor *out,
                                                              const BIWindow &window) {
                                        ...
                                        const int window_end_x = window.x().end();
                                        const int window_start_y = window.y().start();
                                        const int window_end_y = std::min(
                                                window.y().end(), static_cast<int>(in->info()->dimension(1)));
                                        const int window_end_y_multiple_of =
                                                        ((window_end_y - window_start_y) / window_step_y) *
                                                        window_step_y;
                                        const size_t input_stride_in_bytes = in->info()->strides_in_bytes()[1];
                                        ...
                                        if (left_over_loop_y) {
                                                ...
                                                execute_window_loop(
                                                        window_in,
                                                        [&](const BICoordinates &id) {
                                                                const uint16_t val0 = *(reinterpret_cast<uint16_t *>(
                                                                        input.ptr()));

                                                                // Compute destination address
                                                                const size_t dst_offset_in_bytes =
                                                                                id.y() * sizeof(uint16_t) + id.x() *
                                                                                output_stride_in_bytes;
                                ...
                                void transpose_32bit_elements(const BIITensor *in, BIITensor *out,
                                                              const BIWindow &window) {
                                                        ...
                                                        [&](const BICoordinates &id) {
                                                                ...
                                                                for (; x <= (window_end_x - window_step_x);
                                                                       x += window_step_x) {
                                                                        ...
                                                                        // Transpose 2x4
                                                                        const uint32x4x2_t k0_u32 = {
                                                                                vtrn1q_u32(row0.val[0], row1.val[0]),
                                                                                vtrn2q_u32(row0.val[0], row1.val[0])
                                                                        };
                                                                        const uint32x4x2_t k1_u32 = {
                                                                                vtrn1q_u32(row0.val[1], row1.val[1]),
                                                                                vtrn2q_u32(row0.val[1], row1.val[1])
                                                                        };
                                                                        const uint32x4x2_t k2_u32 = {
                                                                                vtrn1q_u32(row2.val[0], row3.val[0]),
                                                                                vtrn2q_u32(row2.val[0], row3.val[0])
                                                                        };
                                                                        const uint32x4x2_t k3_u32 = {
                                                                                vtrn1q_u32(row2.val[1], row3.val[1]),
                                                                                vtrn2q_u32(row2.val[1], row3.val[1])
                                                                        };
                                                                        const uint32x4x2_t k4_u32 = {
                                                                                vtrn1q_u32(row4.val[0], row5.val[0]),
                                                                                vtrn2q_u32(row4.val[0], row5.val[0])
                                                                        };
                                                                        const uint32x4x2_t k5_u32 = {
                                                                                vtrn1q_u32(row4.val[1], row5.val[1]),
                                                                                vtrn2q_u32(row4.val[1], row5.val[1])
                                                                        };
                                                                        const uint32x4x2_t k6_u32 = {
                                                                                vtrn1q_u32(row6.val[0], row7.val[0]),
                                                                                vtrn2q_u32(row6.val[0], row7.val[0])
                                                                        };
                                                                        const uint32x4x2_t k7_u32 = {
                                                                                vtrn1q_u32(row6.val[1], row7.val[1]),
                                                                        ...
                                                                        // Swap blocks
                                                                        const uint32x4x2_t col0 = {
                                                                                vreinterpretq_u32_u64(k0_u64.val[0]),
                                                                                vreinterpretq_u32_u64(k4_u64.val[0])
                                                                        };
                                                                        const uint32x4x2_t col1 = {
                                                                                vreinterpretq_u32_u64(k1_u64.val[0]),
                                                                                vreinterpretq_u32_u64(k5_u64.val[0])
                                                                        };
                                                                        const uint32x4x2_t col2 = {
                                                                                vreinterpretq_u32_u64(k0_u64.val[1]),
                                                                                vreinterpretq_u32_u64(k4_u64.val[1])
                                                                        };
                                                                        const uint32x4x2_t col3 = {
                                                                                vreinterpretq_u32_u64(k1_u64.val[1]),
                                                                                vreinterpretq_u32_u64(k5_u64.val[1])
                                                                        };
                                                                        const uint32x4x2_t col4 = {
                                                                                vreinterpretq_u32_u64(k2_u64.val[0]),
                                                                                vreinterpretq_u32_u64(k6_u64.val[0])
                                                                        };
                                                                        const uint32x4x2_t col5 = {
                                                                                vreinterpretq_u32_u64(k3_u64.val[0]),
                                                                                vreinterpretq_u32_u64(k7_u64.val[0])
                                                                        };
                                                                        const uint32x4x2_t col6 = {
                                                                                vreinterpretq_u32_u64(k2_u64.val[1]),
                                                                                vreinterpretq_u32_u64(k6_u64.val[1])
                                                                        };
                                                                        const uint32x4x2_t col7 = {
                                                                                vreinterpretq_u32_u64(k3_u64.val[1]),
                                                                                vreinterpretq_u32_u64(k7_u64.val[1])
                                                                        };

                                                                        // Compute destination address
                                                                        const size_t dst_offset_in_bytes =
                                                                                        id.y() * sizeof(uint32_t) + x *
                                                                                        output_stride_in_bytes;
                                        ...
                                        if (left_over_loop_y) {
                                                ...
                                                execute_window_loop(
                                                        window_in,
                                                        [&](const BICoordinates &id) {
                                                                const uint32_t val0 = *(reinterpret_cast<uint32_t *>(
                                                                        input.ptr()));

                                                                // Compute destination address
                                                                const size_t dst_offset_in_bytes =
                                                                                id.y() * sizeof(uint32_t) + id.x() *
                                                                                output_stride_in_bytes;
                ...
                void transpose_32bit_elements(const BIITensor *in, BIITensor *out, const BIWindow &window)
                {
                    ...
                    if (left_over_loop_y)
                    {
                        ...
                        BICpuTransposeKernel::configure(const BIITensorInfo *src, BIITensorInfo *dst) {
                                ...
                                const unsigned int num_elems_processed_per_iteration_y = num_elems_processed(
                                        src->element_size());

                                // Configure kernel window
                                BIWindow win = calculate_max_window(*src, BISteps(num_elems_processed_per_iteration_x,
                        ...
                        void BICpuTransposeKernel::dynamic_configure(const BIITensorInfo *src, BIITensorInfo *dst) {
                                // 注意：这个内核每次迭代处理 16 个元素。
                                // 然而，由于我们在两个维度（X 和 Y）上都使用了剩余循环（left-over for loop），
                                // 因此不能有任何内存的越界读取或写入。
                                // 出于这个原因，num_elems_processed_per_iteration_x 被设置为 1。
                                ...
                                dynamic_origin_max_window(*src, win, BISteps(num_elems_processed_per_iteration_x,
                                                                             num_elems_processed_per_iteration_y));
                        ...
                        BIStatus BICpuTransposeKernel::validate(const BatmanInfer::BIITensorInfo *src,
                                ...
                                BI_COMPUTE_RETURN_ERROR_ON_MSG(
                                        src->element_size() != 1 && src->element_size() != 2 && src->element_size() !=
                                        4,
                                        "Element size not supported");

                                // Validate configured destination
                                if (dst->total_size() != 0) {
                                        const BITensorShape dst_shape =
                                                        misc::shape_calculator::compute_transposed_shape(*src);

                                        BI_COMPUTE_RETURN_ERROR_ON_MISMATCHING_DIMENSIONS(
                                                dst->tensor_shape(), dst_shape);
                        ...
                        void BICpuTransposeKernel::run_op(BatmanInfer::BIITensorPack &tensors,
