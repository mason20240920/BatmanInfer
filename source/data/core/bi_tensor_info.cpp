...
namespace BatmanInfer {
    ...
    size_t
    BITensorInfo::init_auto_padding(const BITensorShape &tensor_shape, size_t num_channels, BIDataType data_type) {
        BI_COMPUTE_ERROR_ON(num_channels == 0);
    ...
    bool BITensorInfo::extend_padding(const BIPaddingSize &padding) {
        ...
        std::tie(_strides_in_bytes, _offset_first_element_in_bytes, _total_size) = calculate_padding_requirements(
                _padding);
    ...
    BITensorInfo::BITensorInfo(const BITensorShape &tensor_shape, size_t num_channels, BIDataType data_type)
            : BITensorInfo() {
        init(tensor_shape, num_channels, data_type);
    }
