 ...
 */
    ...
    void requantize_block_32(const Requantize32 &qp, unsigned int width, unsigned int height,
        ...
        if (qp.per_channel_requant) {
            if (qp.per_channel_left_shifts) {
                requantize_block_32_int<true, true>(qp, width, height, reinterpret_cast<const int32_t *>(input),
                                                    in_stride,
                                                    reinterpret_cast<int8_t *>(output), out_stride, row_bias, col_bias,
                                                    start_col);
            } else {
                requantize_block_32_int<true, false>(qp, width, height, reinterpret_cast<const int32_t *>(input),
                                                     in_stride,
                                                     reinterpret_cast<int8_t *>(output), out_stride, row_bias, col_bias,
                                                     start_col);
            }
            ...
            if (qp.per_layer_left_shift > 0) {
                requantize_block_32_int<false, true>(qp, width, height, reinterpret_cast<const int32_t *>(input),
                                                     in_stride,
                                                     reinterpret_cast<int8_t *>(output), out_stride, row_bias, col_bias,
                                                     start_col);
            } else {
                requantize_block_32_int<false, false>(qp, width, height, reinterpret_cast<const int32_t *>(input),
                                                      in_stride,
                                                      reinterpret_cast<int8_t *>(output), out_stride, row_bias,
                                                      col_bias, start_col);
            }
        }
    }
    ...
    template void requantize_block_32(const Requantize32 &qp, unsigned int width, unsigned int height,
                                      const uint32_t *input, unsigned int in_stride, uint8_t *output,
                                      unsigned int out_stride,
                                      const int32_t *row_bias, const int32_t *col_bias, unsigned int start_col);

    template void requantize_block_32(const Requantize32 &qp, unsigned int width, unsigned int height,
                                      const int32_t *input, unsigned int in_stride, uint8_t *output,
                                      unsigned int out_stride,
                                      const int32_t *row_bias, const int32_t *col_bias, unsigned int start_col);
 ...
 */
    namespace {
        struct row_sum_helpers {
            ...
            void compute_some_rows(unsigned int blocks, const T *input, unsigned int in_stride, int32_t *row_bias,
    ...
    void compute_row_sums(const Requantize32 &qp, unsigned int width, unsigned int height,
        ...
        for (unsigned int row = 0; row < height; row += 4) {
            switch (height - row) {
                default:
                case 4:
                    thehelpers.compute_some_rows<4>(blocks, input + (row * in_stride), in_stride, row_bias + row,
                                                    mask_mode, mask, offset_mul);
                    break;
                case 3:
                    thehelpers.compute_some_rows<3>(blocks, input + (row * in_stride), in_stride, row_bias + row,
                                                    mask_mode, mask, offset_mul);
                    break;
                case 2:
                    thehelpers.compute_some_rows<2>(blocks, input + (row * in_stride), in_stride, row_bias + row,
                                                    mask_mode, mask, offset_mul);
                    break;
                case 1:
                    thehelpers.compute_some_rows<1>(blocks, input + (row * in_stride), in_stride, row_bias + row,
                                                    mask_mode, mask, offset_mul);
                    break;
            }
        }
...
/* Instantiate the two versions for uint8_t and int8_t. */
    ...
    template void
    compute_row_sums(const Requantize32 &, unsigned int, unsigned int, const uint8_t *, unsigned int, int32_t *);
 ...
 * in multithreaded cases where we divide columns across threads) */
    ...
    template void compute_col_sums(const Requantize32 &qp, unsigned int width, unsigned int height, const int8_t *input,
                                   unsigned int in_stride, int32_t *col_bias, unsigned int depth, unsigned int multi,
                                   unsigned int first_col);

    template void
    compute_col_sums(const Requantize32 &qp, unsigned int width, unsigned int height, const uint8_t *input,
                     unsigned int in_stride, int32_t *col_bias, unsigned int depth, unsigned int multi,
                     unsigned int first_col);
