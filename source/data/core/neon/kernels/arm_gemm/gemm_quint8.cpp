...
#include "data/core/neon/kernels/arm_gemm/kernels/a64_hybrid_u8qa_dot_4x16.hpp"
#include "data/core/neon/kernels/arm_gemm/kernels/a64_hybrid_u8qa_mmla_4x16.hpp"
...
namespace BatmanGemm {
    ...
    static const GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32> gemm_quint8_methods[] =
            {
                    ...
                    GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32>::with_estimate(
                            ...
                            [](const GemmArgs &args, const Requantize32 &) {
                                return GemmHybridIndirect<cls_a64_hybrid_u8qa_mmla_4x16, uint8_t, uint8_t, uint8_t, Requantize32>::estimate_cycles<uint8_t>(
                                        args);
                            },
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmHybridIndirect<cls_a64_hybrid_u8qa_mmla_4x16, uint8_t, uint8_t, uint8_t, Requantize32>(
                                        args, qp);
                            }
                    ),
                    GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32>::with_estimate(
                            ...
                            [](const GemmArgs &args, const Requantize32 &) {
                                return GemmInterleavedQuantized<cls_a64_interleaved_u8u32_mmla_8x12, uint8_t, uint8_t, uint8_t>::estimate_cycles<uint8_t>(
                                        args);
                            },
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmInterleavedQuantized<cls_a64_interleaved_u8u32_mmla_8x12, uint8_t, uint8_t, uint8_t>(
                                        args, qp);
                            }
                    ),
                    GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32>::with_estimate(
                            ...
                            [](const GemmArgs &args, const Requantize32 &) {
                                return GemmHybridIndirect<cls_a64_hybrid_u8u32_mmla_6x16, uint8_t, uint8_t, uint8_t, Requantize32, true>::estimate_cycles<uint8_t>(
                                        args);
                            },
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmHybridIndirect<cls_a64_hybrid_u8u32_mmla_6x16, uint8_t, uint8_t, uint8_t, Requantize32, true>(
                                        args, qp);
                            }
                    ),
                    {
                    ...
                    },
                    {
                            ...
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmInterleavedQuantized<cls_a64_gemm_u16_8x12, uint8_t, uint8_t, uint8_t>(
                                        args, qp);
                            },
                    },
                    GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32>::with_estimate(
                            ...
                            [](const GemmArgs &args, const Requantize32 &) {
                                return GemmHybridIndirect<cls_a64_hybrid_u8qa_dot_4x16, uint8_t, uint8_t, uint8_t, Requantize32>::estimate_cycles<uint8_t>(
                                        args);
                            },
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmHybridIndirect<cls_a64_hybrid_u8qa_dot_4x16, uint8_t, uint8_t, uint8_t, Requantize32>(
                                        args, qp);
                            }
                    ),
                    GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32>::with_estimate(
                            ...
                            [](const GemmArgs &args, const Requantize32 &) {
                                return GemmHybridIndirect<cls_a64_hybrid_u8u32_dot_6x16, uint8_t, uint8_t, uint8_t, Requantize32, true>::estimate_cycles<uint8_t>(
                                        args);
                            },
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmHybridIndirect<cls_a64_hybrid_u8u32_dot_6x16, uint8_t, uint8_t, uint8_t, Requantize32, true>(
                                        args, qp);
                            }
                    ),
                    GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32>::with_estimate(
                            ...
                            [](const GemmArgs &args, const Requantize32 &) { return args._ci->has_dotprod(); },
                            [](const GemmArgs &args, const Requantize32 &) {
                                return GemmInterleavedQuantized<cls_a64_gemm_u8_8x12, uint8_t, uint8_t, uint8_t>::estimate_cycles<uint8_t>(
                                        args);
                            },
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmInterleavedQuantized<cls_a64_gemm_u8_8x12, uint8_t, uint8_t, uint8_t>(
                                        args, qp);
                            }
                    ),
                    GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32>::with_estimate(
                            ...
                            nullptr,
                            [](const GemmArgs &args, const Requantize32 &) {
                                return GemmInterleavedQuantized<cls_a64_gemm_u8_4x4, uint8_t, uint8_t, uint8_t>::estimate_cycles<uint8_t>(
                                        args);
                            },
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new GemmInterleavedQuantized<cls_a64_gemm_u8_4x4, uint8_t, uint8_t, uint8_t>(
                                        args, qp);
                            }
                    ),
                    {
                            ...
                            [](const GemmArgs &args, const Requantize32 &qp) {
                                return new QuantizeWrapper<uint8_t, uint8_t, uint32_t>(args, qp);
                            }
                    },
                    {
                            GemmMethod::DEFAULT,
    ...
    template<>
    const GemmImplementation<uint8_t, uint8_t, uint8_t, Requantize32> *
    gemm_implementation_list<uint8_t, uint8_t, uint8_t, Requantize32>() {
        return gemm_quint8_methods;
    }

    template UniqueGemmCommon<uint8_t, uint8_t, uint8_t>
    gemm<uint8_t, uint8_t, uint8_t, Requantize32>(const GemmArgs &args, const Requantize32 &os);

    template bool
    has_opt_gemm<uint8_t, uint8_t, uint8_t, Requantize32>(WeightFormat &weight_format, const GemmArgs &args,
                                                          const Requantize32 &os);

    template KernelDescription
    get_gemm_method<uint8_t, uint8_t, uint8_t, Requantize32>(const GemmArgs &args, const Requantize32 &os);

    template std::vector<KernelDescription>
    get_compatible_kernels<uint8_t, uint8_t, uint8_t, Requantize32>(const GemmArgs &args, const Requantize32 &os);
