...
#include "data/core/neon/kernels/arm_gemm/transform.hpp"
...
// _full version doesn't need to worry about any padding.
        static inline void transpose_block_32_full(const uint8_t *__restrict in_ptr0, const uint8_t *__restrict in_ptr1,
                                                   const uint8_t *__restrict in_ptr2, const uint8_t *__restrict in_ptr3,
                                                   uint8_t *__restrict out_ptr, long output_stride) {
            uint32x4_t inputs[4];
            uint32x4_t inters[4];
...
// multiple of 4 there will some padding in each 4-block)
        static inline void
        transpose_block_32_part(const uint8_t *in_ptr0, const uint8_t *in_ptr1, const uint8_t *in_ptr2,
                                const uint8_t *in_ptr3, uint8_t *out_ptr, long bytes_in, long output_stride) {
            uint32x4_t inputs[4];
            uint32x4_t inters[4];
            uint32x4_t outputs[4];
...
// The total interleave depth for a given run is therefore BLOCKS * interleave_multiple * 4.
        ...
        void a64_interleave_1x4(uint8_t *out, const uint8_t *in, long width, long in_stride, long height,
                                long interleave_multiple) {
            ...
            for (long y0_base = 0; y0_base < height; y0_base += total_interleave_depth) {
                ...
                for (long block = 0; block < interleave_multiple; block++) {
                    ...
                    uint8_t *out_ptr = out + (block * loop_interleave_depth *
                                              4); // 4 is the blocking depth (we interleave 4 bytes at a time from each input)
                    ...
                    while (bytes_left >= 16) { // 16 is the vector length in bytes
                        Unroll<BLOCKS>::run([&](unsigned u) {
                            transpose_block_32_full(in_ptrs[u * 4 + 0], in_ptrs[u * 4 + 1], in_ptrs[u * 4 + 2],
                                                    in_ptrs[u * 4 + 3],
                                                    out_ptr + 16 * u,
                                                    total_interleave_depth * 4); // 4 is the blocking depth
                        });
                    ...
                    if (bytes_left) {
                        Unroll<BLOCKS>::run([&](unsigned u) {
                            transpose_block_32_part(in_ptrs[u * 4 + 0], in_ptrs[u * 4 + 1], in_ptrs[u * 4 + 2],
                                                    in_ptrs[u * 4 + 3],
