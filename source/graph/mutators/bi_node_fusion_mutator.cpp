...
#include "graph/mutators/bi_node_fusion_mutator.h"
...
namespace detail {
    ...
    void transfer_driving_nodes_and_remove_old_node(BIGraph &g, BIINode *new_node, BIINode *old_node,
        bool add_output_tensor)
    {
        ...
        std::vector<BINodeIdxPair> last_driving_nodes = get_driving_nodes(*old_node);
    ...
    void fuse_convolution_with_batch_normalization(BIGraph &g, const BIEdge *output_edge)
    {
        BI_COMPUTE_ERROR_ON(output_edge == nullptr);

        auto *conv_node =
            BatmanInfer::utils::cast::polymorphic_downcast<ConvolutionLayerNode *>(output_edge->producer());
        auto *bn_node =
            BatmanInfer::utils::cast::polymorphic_downcast<BatchNormalizationLayerNode *>(output_edge->consumer());
        ...
        BI_COMPUTE_LOG_GRAPH_VERBOSE("Fusing convolution node with ID : "
        ...
        if (conv_node->output(0)->accessor() == nullptr)
        {
            ...
            BIFastMathHint fast_math_hint = conv_node->fast_math_hint();
            ...
            fused_node->set_common_node_parameters(
                BINodeParams{conv_node->name() + "+" + bn_node_name, assigned_target});
    ...
    void fuse_depthwise_convolution_with_batch_normalization(BIGraph &g, const BIEdge *output_edge)
    {
        BI_COMPUTE_ERROR_ON(output_edge == nullptr);

        auto *depth_conv_node =
            BatmanInfer::utils::cast::polymorphic_downcast<DepthwiseConvolutionLayerNode *>(output_edge->producer());
        auto *bn_node =
            BatmanInfer::utils::cast::polymorphic_downcast<BatchNormalizationLayerNode *>(output_edge->consumer());

        BI_COMPUTE_LOG_GRAPH_VERBOSE("Fusing depthwise convolution node with ID : "
            << output_edge->producer_id() << " with BatchNormalization Layer node with ID : "
            << output_edge->consumer_id() << std::endl);
        ...
        {
            const BITarget assigned_target = depth_conv_node->assigned_target();
            ...
            fused_node->set_assigned_target(assigned_target);
            fused_node->set_common_node_parameters(
                BINodeParams{depth_conv_node->name() + "+" + bn_node_name, assigned_target});
        ...
        {
            BI_COMPUTE_LOG_GRAPH_VERBOSE("Prevented fusion of depthwise convolution with batch normalization due to the "
                                         "presence of an output accessor\n");
        }
    }

    template <typename N>
    void fuse_node_with_activation(BIGraph                    &g,
                                   const BIEdge               *output_edge,
                                   const std::set<Activation> &supported_fused_activations)
    {
        BI_COMPUTE_ERROR_ON(output_edge == nullptr);

        auto *n_node   = BatmanInfer::utils::cast::polymorphic_downcast<N *>(output_edge->producer());
        auto *act_node = BatmanInfer::utils::cast::polymorphic_downcast<ActivationLayerNode *>(output_edge->consumer());

        BI_COMPUTE_ERROR_ON(act_node->output(0) == nullptr || n_node->output(0) == nullptr);
        ...
        if (n_node->type() == BINodeType::EltwiseLayer && !is_data_type_float(n_node->output(0)->desc().data_type))
        {
            return;
        }

        BI_COMPUTE_LOG_GRAPH_VERBOSE("Fusing node with ID : " << output_edge->producer_id()
    ...
    template <typename N>
    void fuse_pad_with_convolution(BIGraph &g, const BIEdge *output_edge)
    {
        auto *pad_node  = BatmanInfer::utils::cast::polymorphic_downcast<PadLayerNode *>(output_edge->producer());
        auto *conv_node = BatmanInfer::utils::cast::polymorphic_downcast<N *>(output_edge->consumer());

        const BIEdge *input_edge = pad_node->input_edge(0);
        if (input_edge != nullptr && input_edge->tensor() != nullptr && pad_node->output(0)->accessor() == nullptr &&
            pad_node->pad_value().get<float>() == 0.0)
        {
            const BIDataLayout  layout     = input_edge->tensor()->desc().layout;
            ...
            if (is_padding_in_height_or_width(layout, padding_list))
            {
                ...
                const BIPadStrideInfo new_conv_info(
                ...
                std::vector<BINodeIdxPair> pad_driver_nodes = get_driver_nodes(*pad_node);
    ...
    void fuse_layer(BIGraph &g, std::function<bool(BIINode &)> const &prec, const F fuse_fcn, Args &&...optional_arguments)
    {
    ...
    const char *BINodeFusionMutator::name()
    {
        return "BINodeFusionMutator";
    }

    BIIGraphMutator::MutationType BINodeFusionMutator::type() const
    {
        return BIIGraphMutator::MutationType::Backend;
    }

    void BINodeFusionMutator::mutate(BIGraph &g)
    {
        ...
        // Preconditions
        auto empty_prec     = [](BIINode &) { return true; };
        auto cl_target_prec = [](BIINode &n) { return n.assigned_target() == BITarget::CL; };
        auto qs8_prec       = [&g](BIINode &n)
        {
            BI_COMPUTE_ERROR_ON(n.output(0) == nullptr);
            ...
            const bool same_qinfo =
                n.output(0)->desc().quant_info == output_edge->producer()->output(0)->desc().quant_info;
            const bool output_qasymm8 = n.output(0)->desc().data_type == BIDataType::QASYMM8;
        ...
        detail::fuse_layer<PadLayerNode, ConvolutionLayerNode>(
            g, empty_prec, detail::fuse_pad_with_convolution<ConvolutionLayerNode>);
