...
//            _gemm_state_sum_layer(),
    ...
    void BINEAttentionLayer::configure(const BatmanInfer::BIITensor *input,
        ...
        // 结果判断
        BI_COMPUTE_ERROR_ON_NULLPTR(input, weights, bias, output);  // 输入的参数是否为空
        ...
        // 配置中间张量输出
        BITensorShape normal_shape = BITensorShape(_max_seq_len, _hidden_size); // 默认输入和输出
        ...
        _copy_f.configure(&_norm_output, output);
//        // 转置的输出shape
...
//
//        // 第三进行结果的Split
//        BITensorShape split_shape = BITensorShape(16, 768);
//
//        // 第一层切分的reshape结构
//        BITensorShape split_layer_0_shape = BITensorShape(16, 12, 64);
//
//        // 第一层切分的transpose结构
//        BITensorShape split_layer_0_trans_shape = BITensorShape(64, 16, 12);
//
//        // 第二个分支的split的transpose不同
//        BITensorShape split_linear_1_trans_shape = BITensorShape(16, 64, 12);
//
//        // 合并分支代码
//        BITensorShape mat_mul_shape = BITensorShape(16, 16, 12);
//
//        // 最后转置的变成了三维
//        BITensorShape final_transpose_shape = BITensorShape(16, 12, 64);
        ...
        // 归一层输出
//        _layer_nor_output.allocator()->init(BITensorInfo(split_shape, 1, input->info()->data_type()));
//
...
//        _memory_group.manage(&_layer_nor_output);
...
//        _layer_nor_output.allocator()->allocate();



        // 进行切分层的结果输入和输出
//        _normalization_layer.configure(input, &_layer_nor_output, norm_info); // 归一化
//        _gemm_state_f.configure(weights, &_layer_nor_output, bias, &_gemm_output, 1.f, 1.f);
//        std::vector<BIITensor *> outputs = {&_split_result_0, &_split_result_1, &_split_result_2};
//        _split_layer.configure(&_gemm_output, outputs, 1);
...
//        _gemm_state_sum_layer.configure(weights_second, &_reshape_sum_tensor, bias_second, &_gemm_sum_output, 1.f, 1.f);
