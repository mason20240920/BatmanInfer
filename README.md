...
> **halide_dimension_t**解释
>
> ```c++
> // 1. min: 表示0维度起始索引为0
> //   * Halide的索引范围从`min`开始, 索引范围为[min, min + extent - 1]
> //   * 在这里`min = 0`表示channel的索引范围是`[0, channels - 1]`
> // ----
> // 2. extent = channels
> //  * 表示第0维的长度，即通道数
> //  * `extent`是该维度所包含的元素数量, `extent`等于`channels`, 表示张量有多少通道
> // 3. stride = rows * cols;
> //  * 表示跨越第0维（通道维度）一个单位所需的存储单元数
> //  * 在这里, `stride` 是每个通道占用的存储单元数，即每个通道包含的元素总数
> //  * 每个通道 = (rows * cols), stride = rows * cols
> // 4.flags = 0
> //   没有实际意义
> h_data_.dim[0] = {0,
>                   channels,
